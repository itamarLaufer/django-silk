...
from silk.models import Request, SQLQuery, Profile, APICall
from silk.utils.pagination import _page

__author__ = 'mtford'


class APICallsView(View):
    ...
    def get(self, request, *_, **kwargs):
        ...
        if request_id:
            ...
            query_set = APICall.objects.filter(request=silk_request).order_by('-start_time')
            for q in query_set:
        ...
        if profile_id:
            ...
            page = _page(request, p.api_calls.order_by('-start_time').all())
            context['profile'] = p
        if not (request_id or profile_id):
            raise KeyError('no profile_id or request_id')
        # noinspection PyUnboundLocalVariable
        context['items'] = page
        return render(request, 'silk/api_calls.html', context)
